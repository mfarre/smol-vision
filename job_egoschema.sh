#!/bin/bash
#SBATCH --job-name=smolvlm_eval-egoschema  # Job name
#SBATCH --nodes=1                       # Number of nodes
#SBATCH --ntasks=1                      # Number of tasks (typically one per node)
#SBATCH --gres=gpu:1                   # Number of GPUs per node
##SBATCH --mem=128G                      # Memory per node
#SBATCH --output=/fsx/miquel/smol-vision/eval_logs/evals-egoschema-%x-%j.out            # Standard output log
#SBATCH --error=/fsx/miquel/smol-vision/eval_logs/evals-egoschema-%x-%j.err             # Standard error log
...
# Run eval scripts
srun bash eval_video_mcqa_egoschema.sh

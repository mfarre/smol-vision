...
class SmolVLMContextTester:
    ...
    def test_context_window(self, images_folder: str, step_size: int = 2) -> List[Tuple[int, float]]:
...
def main():
    ...
    # Print results
    print("\nResults:")
    print("Number of Images | Perplexity")
    print("-" * 30)
    for num_images, perplexity in results:
        print(f"{num_images:^14} | {perplexity:.4f}")

    # Save results to file
    output_file = "context_test_results.txt"
    with open(output_file, "w") as f:
        f.write("Number of Images | Perplexity\n")
        f.write("-" * 30 + "\n")
        for num_images, perplexity in results:
            f.write(f"{num_images:^14} | {perplexity:.4f}\n")
